...
const SelectionSortVisualizer = () => {
  ...
  const SelectionSort = async () => {
    ...
    logStep("Starting Selection Sort...", "start", arr, [], []);
    ...
    for (let i = 0; i < len - 1; i++) {
      ...
      let minIndex = i;
      setHighlightedIndices([minIndex]);
      logStep(`Pass ${i + 1}: Finding minimum in unsorted portion`, "pass", arr, [minIndex], [...sortedIndices]);
      ...
      for (let j = i + 1; j < len; j++) {
        ...
        logStep(`Comparing ${arr[j]} with current minimum ${arr[minIndex]}`, "compare", arr, [minIndex, j], [...sortedIndices]);
        await sleep(1000 - speed * 10);

        if (arr[j] < arr[minIndex]) {
          minIndex = j;
          setHighlightedIndices([minIndex]);
          logStep(`New minimum found at index ${minIndex} (${arr[minIndex]})`, "new-min", arr, [minIndex], [...sortedIndices]);
          await sleep(1000 - speed * 10);
        }
      }
      ...
      if (minIndex !== i) {
        ...
        logStep(`Swapped ${arr[minIndex]} with ${arr[i]}`, "swap", arr, [i, minIndex], [...sortedIndices]);
        await sleep(1000 - speed * 10);
      }

      setSortedIndices(prev => [...prev, i]);
      logStep(`Element at index ${i} (${arr[i]}) is now in final position`, "sorted", arr, [], [...sortedIndices, i]);
      await sleep(1000 - speed * 10);
    }
  ...
  const getStepStyle = (type) => {
    switch (type) {
      case "start": return "bg-indigo-600/90";
      case "pass": return "bg-purple-600/90";
      case "compare": return "bg-yellow-500/90";
      case "new-min": return "bg-pink-600/90";
      case "swap": return "bg-red-600/90";
      case "sorted": return "bg-green-600/90";
      case "finish": return "bg-teal-600/90";
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 md:p-8 flex flex-col items-center">
      ...
      <header className="w-full max-w-6xl mb-8 text-center">
        ...
        <p className="text-gray-400 max-w-2xl mx-auto">
          Visualize how the Bubble Sort algorithm works step by step with complexity analysis.
        </p>
      </header>
      ...
      <main className="w-full max-w-6xl space-y-6">
        ...
        <section className="bg-gray-800/80 backdrop-blur-sm rounded-xl p-6 border border-gray-700 shadow-lg">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-gray-700/50 p-4 rounded-lg border border-gray-600">
              <h3 className="text-sm text-gray-400 mb-1">Swaps</h3>
              <div className="flex items-baseline gap-2">
